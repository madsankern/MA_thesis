...
def compute_wq(t,sol,par,compute_q=False):
    ...
    inv_w = sol.inv_w[t]
    q = sol.q[t]
    # print(np.shape(inv_w))
    # print(inv_w)

    # loop over outermost post-decision state
    for i_p in range(par.Np): # prange # Loop over possible values of y here
        ...
        # Already compute y_plus here? (Call it p_plus for now)
        # p_plus = par.P[i_p]*par.grid_p[0] + (1-par.P[i_p])*par.grid_p[1] # Conditional expectation, write as a function
        ...
        for i_n in range(par.Nn): # Loop over state of durables
            ...
            # p_plus = trans.p_plus_func(p,psi_plus,par) # Model for the permament income

            p_plus = par.P[i_p][0]*par.grid_p[0] + par.P[i_p][1]*par.grid_p[1] # Conditional expectation, write as a function, works fine
            ...
            weight = 1 # Add Markov probabilities here.
...
#                 # iv. weight of each income shock
#                 weight = psi_plus_w*xi_plus_w # Add Markov probabilities here.

#                 # v. next-period cash-on-hand and total resources
#                 for i_a in range(par.Na):
        
#                     m_plus[i_a] = trans.m_plus_func(par.grid_a[i_a],p_plus,xi_plus,par,n)
#                     x_plus[i_a] = trans.x_plus_func(m_plus[i_a],n_plus,par) # No need to update this
