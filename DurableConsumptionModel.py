...
class DurableConsumptionModelClass(ModelClass): # Rename
    ...
    def simulate_path_prep(self):
        ...
        # d. Income states
        sim_path.state = np.zeros((par.sim_T + par.path_T,par.simN),dtype=np.int_) 
...
#         # d. Path for aggregate states
#         par.path_R = np.zeros(shape=(par.sim_T + par.path_T)) # for impulse response
#         par.path_ph = np.repeat(par.ph,par.sim_T + par.path_T) # House price sequence
...
#         # d. Distance between iterations
#         sol.dist = np.nan*np.zeros(par.T-1)
...
#                 # iii. Distance from previous iteration = max over difference for keeper and adjuster
#                 if t < par.T-1:
#                     dist_keep = np.abs(np.max(sol.inv_v_keep[t+1,:,:,:,:] - sol.inv_v_keep[t,:,:,:,:]))
#                     dist_adj = np.abs(np.max(sol.inv_v_adj[t+1,:,:,:] - sol.inv_v_adj[t,:,:,:]))
#                     sol.dist[t] = np.maximum(dist_keep,dist_adj)
...
#         # a. Random initial allocation of housing and cash on hand
#         sim.d0[:] = np.random.choice(par.grid_n,size=par.simN) # Initial housing (discrete values). Use Binomial for initial draw
#         sim.a0[:] = par.mu_a0*np.random.lognormal(mean=1.3,sigma=par.sigma_a0,size=par.simN) # initial cash on hand
...
#         # c. States and choices
#         sim_shape_path = (par.sim_T + 50,par.simN) # update this
#         sim_path.y = np.zeros(sim_shape_path)
#         sim_path.m = np.zeros(sim_shape_path)
#         sim_path.n = np.zeros(sim_shape_path)
#         sim_path.discrete = np.zeros(sim_shape_path,dtype=np.int)
...
#         # d. Income states
#         sim_path.state = np.zeros((par.sim_T+50,par.simN), dtype=np.int_) 
