...
def optimal_choice(t,p,n,m,discrete,d,c,a,sol,par): # Calculate the optimal choice
    ...
    # inv_v_keep = linear_interp.interp_1d(par.grid_m,sol.inv_v_keep[t,],)
    inv_v_adj = linear_interp.interp_2d(par.grid_p,par.grid_x,sol.inv_v_adj[0,0],p,x)    
    adjust = inv_v_adj > inv_v_keep # Determine whether adjusting is optimal
    ...
    if adjust:
        ...
        tot = par.ph*d[0]+c[0] # update with house price
        if tot > x: # If wanting to buy more than you can afford
            d[0] *= x/tot  # Ensure that c and d sum to x
            c[0] *= x/tot
            a[0] = 0.0
        else:
            a[0] = x - tot
            
    else: # if keeping is optimal
        ...
        c[0] = linear_interp.interp_3d(
            ...
            p,n,m) # Compute c by interpolation
...
# @njit            
# def optimal_choice_2d(t,p,n1,n2,m,discrete,d1,d2,c,a,sol,par): # CAN BE REMOVED
